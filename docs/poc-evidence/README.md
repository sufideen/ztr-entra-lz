...
| Standing role assignments on the **CI identity** | 0 | 2 — subscription-scope Contributor + the narrow custom "ztr-entra-lz CI Authorization Writer" role (see README "One-time bootstrap"). Deliberately excludes `Microsoft.Authorization/roleAssignments/*`, but is still a standing (non-PIM-eligible) grant on an unattended identity, worth flagging against the "0 standing assignments" Zero Trust claim rather than glossing over. Drift beyond these two (e.g. a hand-added `Owner`/`Reader` grant) is detected by the `CI identity holds only the documented standing role assignments` Pester test and flagged early by `scripts/azure/reconcile-ci-identity-roles.ps1`, which the `deploy` job now runs report-only before that test; fixing it requires `-Fix` under an admin identity (not the CI identity itself, which deliberately cannot touch role assignments — see README "Troubleshooting: post-deploy test failures") |
